...
SHIRT_LIKE_SVG_base = emma-watson-wandless--3rd-tense--a4-shirt-like
SHIRT_LIKE_SVG = $(SHIRT_LIKE_SVG_base).svg
WIDTH_SHIRT_LIKE_SVG_EXP_BASE = $(SHIRT_LIKE_SVG_base)--width
WIDTH_SHIRT_LIKE_PNG = $(WIDTH_SHIRT_LIKE_SVG_EXP_BASE).png
WIDTH_SHIRT_LIKE_WEBP = $(WIDTH_SHIRT_LIKE_SVG_EXP_BASE).webp
...
all: $(SHIRT_PNG)
all: $(WIDTH_SHIRT_LIKE_PNG)
all: $(WIDTH_SHIRT_LIKE_WEBP)
all: $(WIDTH_SHIRT_PNG)
...
$(WIDTH_SHIRT_LIKE_PNG): $(SHIRT_LIKE_SVG) $(PHOTO_DEST)
	inkscape --export-type=png --export-filename=$@ --export-width=$(WIDTH) --export-area-page $<
	optipng $@

$(WIDTH_SHIRT_LIKE_WEBP): $(WIDTH_SHIRT_LIKE_PNG)
	gm convert $< $@
